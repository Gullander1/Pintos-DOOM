...
static char const *filesys_bdev_name;
static char const *scratch_bdev_name;
#ifdef VM
...
static void locate_block_device(enum block_type, char const *name);
#endif
...
int main(void)
{
	...
	/* FIX */
	printf("DEBUG: Setting init_ram_pages to 16384 (64 MB)...\n");
    ...
    wm_create_window(50, 50, 644, 404, "DOOM", 0x0000);

  	vesa_update_display(512, 384);
    thread_create("gui_manager", PRI_MAX, gui_thread_func, NULL);
    /* ----------------------- */
...
#endif
#ifdef LEAKCHECK
		"  -L				  Enable memory leak checker (false postivites possible).\n"
#endif
	);
...
/* Figures out what block device to use for the given ROLE: the
  ...
  ROLE. */
static void locate_block_device(enum block_type role, char const *name)
{
