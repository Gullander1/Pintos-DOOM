...
static inline unsigned pg_ofs(void const *va)
{
	return (uintptr_t) va & PGMASK;
}

/* Virtual page number. */
static inline uintptr_t pg_no(void const *va)
{
	return (uintptr_t) va >> PGBITS;
}

/* Round up to nearest page boundary. */
static inline void *pg_round_up(void const *va)
{
	return (void *) (((uintptr_t) va + PGSIZE - 1) & ~PGMASK);
}

/* Round down to nearest page boundary. */
static inline void *pg_round_down(void const *va)
{
...
static inline bool is_user_vaddr(void const *vaddr)
{
	return vaddr < PHYS_BASE;
}

/* Returns true if VADDR is a kernel virtual address. */
static inline bool is_kernel_vaddr(void const *vaddr)
{
	return vaddr >= PHYS_BASE;
}
...
/* Returns physical address at which kernel virtual address VADDR
  is mapped. */
static inline uintptr_t vtop(void const *vaddr)
{
