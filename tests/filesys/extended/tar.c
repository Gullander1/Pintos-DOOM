...
static bool make_tar_archive(char const *archive_name, char *files[], size_t file_cnt);
...
static bool
archive_ordinary_file(char const *file_name, int file_fd, int archive_fd, bool *write_error);
...
static bool
write_header(char const *file_name, enum ustar_type, int size, int archive_fd, bool *write_error);

static bool do_write(int fd, char const *buffer, int size, bool *write_error);

static bool make_tar_archive(char const *archive_name, char *files[], size_t file_cnt)
{
	static char const zeros[512];
	int archive_fd;
	bool success = true;
