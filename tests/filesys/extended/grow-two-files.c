...
static void write_some_bytes(char const *file_name, int fd, char const *buf, size_t *ofs)
{
	if (*ofs < FILE_SIZE) {
		...
		if (ret_val != block_size) {
			fail(
				"write %zu bytes at offset %zu in \"%s\" returned %zu", block_size, *ofs, file_name, ret_val
			);
		}
		*ofs += block_size;
	}
