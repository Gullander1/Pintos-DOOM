...
static void vmsg(char const *format, va_list args, char const *suffix)
{
...
void exec_children(char const *child_name, pid_t pids[], size_t child_cnt)
{
	size_t i;
...
void check_file_handle(int fd, char const *file_name, void const *buf_, size_t size)
{
	char const *buf = buf_;
	size_t ofs = 0;
	size_t file_size;
...
void check_file(char const *file_name, void const *buf, size_t size)
{
	int fd;
...
void compare_bytes(
	void const *read_data_,
	void const *expected_data_,
	size_t size,
	size_t ofs,
	char const *file_name
)
{
	uint8_t const *read_data = read_data_;
	uint8_t const *expected_data = expected_data_;
	size_t i, j;
	size_t show_cnt;
