...
// HELP function
int doom_strncasecmp(const char *s1, const char *s2, int n) {
    for (int i = 0; i < n; i++) {
        char c1 = s1[i]; if (c1 >= 'a' && c1 <= 'z') c1 -= 32;
        char c2 = s2[i]; if (c2 >= 'a' && c2 <= 'z') c2 -= 32;
        if (c1 != c2) return 1; // 1 = Does not match
        if (c1 == 0) return 0;
    }
    return 0; // 0 = MATCHING!
}
...
int W_CheckNumForName (char* name)
{
    ...
    if (lumphash != NULL)
    {
        ...
        {
            if (!doom_strncasecmp(lump_p->name, name, 8))
            {
    ...
    else
    {
        ...
        {
            if (!doom_strncasecmp(lumpinfo[i].name, name, 8))
            {
                return i;
            }
        }
...
void *W_CacheLumpNum(int lumpnum, int tag)
{
    ...
    if ((unsigned)lumpnum >= numlumps)
    {
        ...
        static int safe_dummy[1024] = {0}; 
        
        safe_dummy[0] = 0x00010001;
        safe_dummy[1] = 0x00000000;
        safe_dummy[2] = 12;        
        safe_dummy[3] = 0x000000FF;
