...
{ 
    ...
    // Fix for Pintos OS
    if (gamekeydown[key_fire] || gamekeydown['f'] || gamekeydown[' '] || mousebuttons[mousebfire] || joybuttons[joybfire])
	cmd->buttons |= BT_ATTACK; 
 
    if (gamekeydown[key_use] || gamekeydown['e'] || joybuttons[joybuse] || mousebuttons[mousebuse])
    { 
	cmd->buttons |= BT_USE;
...
/* Changes has been meed to able to not use float. */
