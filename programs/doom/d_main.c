...
void D_Display (void)
{
    ...
    if (gamestate != wipegamestate) {
        wipe = true;
        //printf("DEBUG: Calling wipe_StartScreen\n");
    ...
    //printf("DEBUG: Buffered drawing (gamestate=%d)\n", gamestate);
    switch (gamestate) {
        case GS_LEVEL:
        if (!gametic) break;
        if (automapactive) AM_Drawer ();
        if (wipe || (viewheight != 200 && fullscreen) ) redrawsbar = true;
        if (inhelpscreensstate && !inhelpscreens) redrawsbar = true;
    ...
    //printf("DEBUG: Calling I_UpdateNoBlit\n");
    I_UpdateNoBlit ();

    if (gamestate == GS_LEVEL && !automapactive && gametic) {
        //printf("DEBUG: Calling R_RenderPlayerView (3D Engine!)\n");
    ...
    if (gamestate == GS_LEVEL && !automapactive && scaledviewwidth != 320) {
        ...
        if (borderdrawcount) { R_DrawViewBorder (); borderdrawcount--; }
    }
    ...
    //printf("DEBUG: Calling M_Drawer\n");
    M_Drawer ();
    NetUpdate ();

    if (!wipe) {
        //printf("DEBUG: Calling I_FinishUpdate (no wipe)\n");
        I_FinishUpdate ();
        return;
    }

    //printf("DEBUG: Calling wipe_EndScreen\n");
    wipe_EndScreen(0, 0, SCREENWIDTH, SCREENHEIGHT);

    wipestart = I_GetTime () - 1;

    //printf("DEBUG: Starting screen wipe loop\n");
    do {
        do {
            nowtime = I_GetTime ();
...
//    W_PrintDirectory();
    ...
    "  |                      | |/ /\\ \\_/ \\ \\_/ | |  | |                     |\n"
    "  |                      |___/  \\___/ \\___/\\_|  |_/                     |\n"
    "  | _   _____________ _   _ _____ _      ______  ___  _   _ _____ _____ |\n"
    "  || | / |  ___| ___ | \\ | |  ___| |     | ___ \\/ _ \\| \\ | |_   _/  __ \\|\n"
    "  || |/ /| |__ | |_/ |  \\| | |__ | |     | |_/ / /_\\ |  \\| | | | | /  \\/|\n"
    "  ||    \\|  __||    /| . ` |  __|| |     |  __/|  _  | . ` | | | | |    |\n"
    "  || |\\  | |___| |\\ \\| |\\  | |___| |____ | |   | | | | |\\  |_| |_| \\__/\\|\n"
    "  |\\_| \\_\\____/\\_| \\_\\_| \\_\\____/\\_____/ \\_|   \\_| |_\\_| \\_/\\___/ \\____/|\n"
    ...
    "  |                                                                     |\n"
    "  |                       DOOM: Kernel Panic Edition                    |\n"
    "  |              PINTOS OS. 2026 Modified by Kevin Gullander            |\n"
    "  |                                                                     |\n"
    "  '---------------------------------------------------------------------'\n"
);
