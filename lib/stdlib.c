...
static int compare_thunk(void const *a, void const *b, void *aux)
{
	int (**compare)(void const *, void const *) = aux;
	return (*compare)(a, b);
}
...
{
	unsigned char const *first = array;
	unsigned char const *last = array + size * cnt;

	while (first < last) {
		size_t range = (last - first) / size;
		unsigned char const *middle = first + (range / 2) * size;
