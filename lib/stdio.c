...
int vsnprintf(char *buffer, size_t buf_size, char const *format, va_list args)
{
...
/* Like printf(), except that output is stored into BUFFER,
  ...
  have been written to BUFFER, not including a null terminator,
  had there been enough room. */
int snprintf(char *buffer, size_t buf_size, char const *format, ...)
{
	va_list args;
	int retval;
...
struct integer_base {
	int base;           /* Base. */
	char const *digits; /* Collection of digits. */
...
static char const *parse_conversion(char const *format, struct printf_conversion *, va_list *);
...
void __vprintf(char const *format, va_list args, void (*output)(char, void *), void *aux)
{
	for (; *format != '\0'; format++) {
...
static char const *parse_conversion(char const *format, struct printf_conversion *c, va_list *args)
{
...
/* Wrapper for __vprintf() that converts varargs into a
  va_list. */
void __printf(char const *format, void (*output)(char, void *), void *aux, ...)
{
	va_list args;
...
/* Dumps the SIZE bytes in BUF to the console as hex bytes
  ...
  alongside. */
void hex_dump(uintptr_t ofs, void const *buf_, size_t size, bool ascii)
{
	uint8_t const *buf = buf_;
	size_t const per_line = 16; /* Maximum bytes per line. */
...
void print_human_readable_size(uint64_t size)
{
	if (size == 1) {
		...
		static char const *factors[] = { "bytes", "kB", "MB", "GB", "TB", NULL };
		char const **fp;
