...
{
	uint8_t const *header = (uint8_t const *) h;
	unsigned int chksum;
	size_t i;
	...
	for (i = 0; i < USTAR_HEADER_SIZE; i++) {
		/* The ustar checksum is calculated as if the chksum field
		  were all spaces. */
		size_t const chksum_start = offsetof(struct ustar_header, chksum);
		size_t const chksum_end = chksum_start + sizeof h->chksum;
...
/* Drop possibly dangerous prefixes from FILE_NAME and return the
  ...
  literal. */
static char const *strip_antisocial_prefixes(char const *file_name)
{
...
static bool parse_octal_field(char const *s, size_t size, unsigned long int *value)
{
	size_t ofs;
...
/* Returns true if the CNT bytes starting at BLOCK are all zero,
  false otherwise. */
static bool is_all_zeros(char const *block, size_t cnt)
{
	while (cnt-- > 0) {
		if (*block++ != 0) {
			return false;
...
char const *ustar_parse_header(
	char const header[USTAR_HEADER_SIZE],
	char const **file_name,
	enum ustar_type *type,
	int *size
)
