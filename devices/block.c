...
/* Returns a human-readable name for the given block device
  TYPE. */
char const *block_type_name(enum block_type type)
{
	static char const *block_type_names[BLOCK_CNT] = {
...
struct block *block_get_by_name(char const *name)
{
	struct list_elem *e;
...
/* Write sector SECTOR to BLOCK from BUFFER, which must contain
  ...
  Internally synchronizes accesses to block devices, so external
  per-block device locking is unneeded. */
void block_write(struct block *block, block_sector_t sector, void const *buffer)
{
	check_sector(block, sector);
...
char const *block_name(struct block *block)
{
	return block->name;
}
...
{
	return (
		list_elem != list_end(&all_blocks) ? list_entry(list_elem, struct block, list_elem) : NULL
	);
}
