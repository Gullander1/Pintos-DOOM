...
static char const *partition_type_name(uint8_t);
		         ...
		         || e->type == 0xc5) /* DR-DOS extended partition. */
		{
			...
			if (sector == 0) {
				...
				read_partition_table(
					block, e->offset + primary_extended_sector, primary_extended_sector, part_nr
				);
			}
		}
		else {
...
static char const *partition_type_name(uint8_t type)
{
	...
	static char const *type_names[256] = {
		[0x00] = "Empty",
		[0x01] = "FAT12",
		[0x02] = "XENIX root",
		[0x03] = "XENIX usr",
...
static void partition_write(void *p_, block_sector_t sector, void const *buffer)
{
	struct partition *p = p_;
