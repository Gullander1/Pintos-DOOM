...
static void output_sector(struct channel *, void const *);
...
/* Write sector SEC_NO to disk D from BUFFER, which must contain
  ...
  Internally synchronizes accesses to disks, so external
  per-disk locking is unneeded. */
static void ide_write(void *d_, block_sector_t sec_no, void const *buffer)
{
	struct ata_disk *d = d_;
	struct channel *c = d->channel;
	lock_acquire(&c->lock);
...
static void output_sector(struct channel *c, void const *sector)
{
